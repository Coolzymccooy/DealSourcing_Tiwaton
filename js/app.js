// ================= SAMPLE DATA ===========================
let properties = [
    { id: 1, city: "Manchester", strategy: "buy-to-let", price: 185000, rent: 950, renovation: 0, flipProfit: 0, title: "3 Bed Terrace, Moss Side, Manchester", beds: 3, baths: 1, type: "Terraced", estValue: 210000, description: "Modern 3 bed terrace close to the city centre." },
    { id: 2, city: "London", strategy: "buy-to-flip", price: 400000, rent: 0, renovation: 50000, flipProfit: 70000, title: "2 Bed Flat, Anfield, London", beds: 2, baths: 1, type: "Flat", estValue: 465000, description: "Victorian flat with strong uplift potential after renovation." },
    { id: 3, city: "Birmingham", strategy: "renovate-sell", price: 180000, rent: 0, renovation: 30000, flipProfit: 45000, title: "3 Bed Semi-Detached, Birmingham", beds: 3, baths: 1, type: "Semi-Detached", estValue: 255000, description: "Family semi-detached home with garden." },
    { id: 4, city: "Leeds", strategy: "buy-to-let", price: 150000, rent: 900, renovation: 0, flipProfit: 0, title: "1 Bed Flat near University, Leeds", beds: 1, baths: 1, type: "Flat", estValue: 165000, description: "High-demand student rental close to the university." },
    { id: 5, city: "Liverpool", strategy: "buy-to-let", price: 100000, rent: 700, renovation: 0, flipProfit: 0, title: "2 Bed Starter Home, Liverpool", beds: 2, baths: 1, type: "Terraced", estValue: 120000, description: "Affordable starter home in an upcoming area." },
    { id: 6, city: "Manchester", strategy: "buy-to-flip", price: 250000, rent: 0, renovation: 40000, flipProfit: 60000, title: "3 Bed Family House, Outer Manchester", beds: 3, baths: 2, type: "Semi-Detached", estValue: 330000, description: "Spacious family home in the suburbs." },
    { id: 7, city: "London", strategy: "renovate-sell", price: 350000, rent: 0, renovation: 60000, flipProfit: 90000, title: "2 Bed Flat near Park, London", beds: 2, baths: 1, type: "Flat", estValue: 440000, description: "Bright flat near a large park; strong resale potential." },
    { id: 8, city: "Birmingham", strategy: "buy-to-let", price: 120000, rent: 800, renovation: 0, flipProfit: 0, title: "2 Bed Flat, Transport Links, Birmingham", beds: 2, baths: 1, type: "Flat", estValue: 135000, description: "Well-connected flat near transport links." }
];
...
async function loadBackendDeals() {
    ...
    } catch (error) { console.error("Error loading backend deals:", error); }
}
...
const allStrategyBtns = document.querySelectorAll(".strategy-btn");
const ukMap = document.getElementById("uk-map");
const mapCityIds = ["manchester", "london", "birmingham", "leeds", "liverpool"];
...
let dealsCountEl = document.getElementById("active-deals");
let roiEl = document.getElementById("avg-roi");
let count = 0, avgRoi = 6, dealsTarget = 16, roiTarget = 10.5;
if (dealsCountEl && roiEl) {
    let interval = setInterval(() => {
        if (count < dealsTarget) { count++; dealsCountEl.textContent = `Active Opportunities: ${count}`; }
        if (avgRoi < roiTarget) { avgRoi += 0.2; roiEl.textContent = `Avg ROI: ${avgRoi.toFixed(1)}%`; }
        if (count >= dealsTarget && avgRoi >= roiTarget) clearInterval(interval);
    }, 100);
}
...
document.querySelector('.waitlist-form')?.addEventListener('submit', function (e) {
    ...
    alert(`Added to waitlist: ${email} (demo)`);
    this.reset();
});
...
function renderProperties() {
    ...
    if (!isNaN(minROIVal)) { filtered = filtered.filter(p => calculateROI(p, p.strategy) >= minROIVal); }
    const maxPriceVal = parseFloat(filterMaxPrice?.value);
    if (!isNaN(maxPriceVal)) { filtered = filtered.filter(p => p.price <= maxPriceVal); }
    if (sortBy && sortBy.value !== "none") {
        filtered.sort((a, b) => {
            const roiA = calculateROI(a, a.strategy), roiB = calculateROI(b, b.strategy);
...
// ---- Modal and calculator logic ----
function openCalculator(property) {
    currentProperty = property;
    currentTab = "buy-to-let";
    if (calculatorTitle) calculatorTitle.textContent = property.title || property.city || "Investment Calculator";
    if (dealExternalLink) {
        if (property.link) {
            dealExternalLink.style.display = "inline-flex"; dealExternalLink.href = property.link;
        } else { dealExternalLink.style.display = "none"; dealExternalLink.href = "#"; }
    }
    updateTabs();
    modalBackdrop.classList.add("active");
}
function closeCalculator() { modalBackdrop.classList.remove("active"); currentProperty = null; }
tabs.forEach(tab => { tab.addEventListener("click", () => { currentTab = tab.dataset.tab; updateTabs(); }); });
closeModalBtn.addEventListener("click", closeCalculator);
modalBackdrop.addEventListener("click", event => { if (event.target === modalBackdrop) closeCalculator(); });
function updateTabs() {
    ...
    if (calculatorSubtitle) {
        calculatorSubtitle.textContent =
            currentTab === "buy-to-let" ? "Buy-to-Let Calculator" :
                currentTab === "buy-to-flip" ? "Buy-to-Flip Calculator" :
...
function mapDealToProperty(deal) {
    ...
    const strategy = tags.includes("renovation") || tags.includes("auction") || tags.includes("belowMarketValue")
        ? "buy-to-flip" : "buy-to-let";
    return {
        id: deal.id, city, strategy, price: deal.price || 0, rent: 0, renovation: 0, flipProfit: 0,
        title: deal.title, description: deal.description, estValue: deal.price, dealScore: deal.score,
        tags, source: deal.source, link: deal.link
    };
}

// ---------- HERO/QUIZ/MAP INTERACTIVITY ------
function hideFiltersAndCards() {
    document.getElementById('filters')?.classList.add('hidden');
    propertyListEl?.classList.add('hidden');
}
function showFiltersAndCards() {
    document.getElementById('filters')?.classList.remove('hidden');
    propertyListEl?.classList.remove('hidden');
}
function setupHeroFlow() {
    hideFiltersAndCards();
    allStrategyBtns.forEach(btn => {
        btn.onclick = function () {
            const val = btn.getAttribute('data-strategy');
            if (filterStrategy) filterStrategy.value = val;
    ...
    if (startQuizBtn) startQuizBtn.onclick = function () { quizModal.classList.remove('hidden'); };
    if (quizForm) {
        quizForm.onsubmit = function (e) {
            e.preventDefault();
            const data = new FormData(quizForm);
            if (filterLocation) filterLocation.value = data.get('location');
            if (filterMinROI) filterMinROI.value = data.get('roi');
            if (filterMaxPrice) filterMaxPrice.value = data.get('max-price');
            if (filterStrategy) filterStrategy.value = data.get('strategy');
    ...
    mapCityIds.forEach(city => {
        const el = document.getElementById(city);
        if (el) {
            el.onclick = function () {
                if (filterLocation) filterLocation.value = capitalize(city);
                if (heroIntro) heroIntro.classList.add('hidden');
                if (quizModal) quizModal.classList.add('hidden');
                showFiltersAndCards();
                renderProperties();
...
if (filterStrategy) filterStrategy.addEventListener("change", renderProperties);
if (filterLocation) filterLocation.addEventListener("change", renderProperties);
if (filterMinROI) filterMinROI.addEventListener("input", renderProperties);
if (filterMaxPrice) filterMaxPrice.addEventListener("input", renderProperties);
if (sortBy) sortBy.addEventListener("change", renderProperties);
if (clearFiltersBtn) {
